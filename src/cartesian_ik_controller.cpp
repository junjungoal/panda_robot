...
#include "panda_robot/cartesian_ik_controller.h"
...
{
bool CartesianIKController::init(hardware_interface::RobotHW *hw, ros::NodeHandle &nh)
{
    // subscribe to goto pose for geometry msgs pose
    _target_subscriber = nh.subscribe("/panda_robot/goto_ik_pose", 10, &CartesianIKController::targetCartesianPoseCb, this,
                                      ros::TransportHints().reliable().tcpNoDelay());

    // get parameters and trajectory method selection
    int traj_method_int = 1;
    nh.getParam("/panda_robot/trajectory_method", traj_method_int);
    traj_method = (TrajectoryMethod) traj_method_int;

    std::string arm_id;
    if (!nh.getParam("arm_id", arm_id))
    {
        ROS_ERROR("CartesianIKController: Could not get parameter arm_id");
        return false;
    }
    ...
    {
        ROS_ERROR("CartesianIKController: Could not parse joint names");
        return false;
    }

    if (joint_names.size() != 7)
    {
        ROS_ERROR_STREAM("CartesianIKController: Wrong number of joint names, got "
    ...
    {
        ROS_ERROR("CartesianIKController: Could not initialize position joint interface");
        return false;
    }
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        {
            ROS_ERROR_STREAM(
                "CartesianIKController: Exception getting joint handles: " << e.what());
            return false;
        }
    }

    // hiro panda trac ik service
    _panda_ik_service = panda_robot_controllers::PandaTracIK();
...
void CartesianIKController::update(const ros::Time &time, const ros::Duration &period)
{
...
void CartesianIKController::stopping(const ros::Time &time)
{
    // can't send immediate commands for 0 velocity to robot
}

void CartesianIKController::targetCartesianPoseCb(const geometry_msgs::Pose &target_pose)
{
...
std::array<double, 4> CartesianIKController::catmullRomSpline(const double &tau, const std::array<double,4> &points)
{
...
double CartesianIKController::calcSplinePolynomial(const std::array<double,4> &coeffs, const double &x)
{
...
void CartesianIKController::constantVelCmd(const double &min_step)
{
...
void CartesianIKController::trapezoidVelCmd(const double &max_step, const double &v_change)
{
...
void CartesianIKController::catmullRomSplineVelCmd(const double &norm_pos, const int &joint_num,
...
bool CartesianIKController::_isGoalReached()
{
...
PLUGINLIB_EXPORT_CLASS(panda_robot_controllers::CartesianIKController,
